...
|
+-- lib               # アプリケーションに事前に構成された異なるライブラリを再エクスポートします。基本的にlib直下はフォルダを配置します。
|
...
|
+-- services.server   # アプリケーション全体で共有されるサービス（ビジネスロジック） e.g. データベース操作処理など
|
+-- stores            # グローバルな状態ストア
|
...
|
+-- services    # 特定の機能のサービス（ビジネスロジック） e.g. データベース操作処理など
|
+-- stores      # 特定の機能の状態ストア
|
